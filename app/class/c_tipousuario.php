<?php
/**
 * Administrar la tabla tipousuario
 *
 */
include_once("class/c_interfaz.php");

class c_tipousuario implements c_interfaz 
{
  //atributos base
  var $tipusu_id;
  var $tipusu_nombre;
  var $tipusu_descripcion;
  ...
  { 
    $sql=<<<vic
	    select tipusu_id from tipousuario
	    where tipusu_nombre='$this->tipusu_nombre'
vic;
  ...
  { 
    ...
    $sql=<<<vic
	    select tipusu_id,tipusu_nombre,tipusu_descripcion 
	    from tipousuario
	    where tipusu_id=$id
vic;
	...
	{
	  $this->tipusu_id=$rs->fields[0];
	  $this->tipusu_nombre=$rs->fields[1];
	  $this->tipusu_descripcion=$rs->fields[2];
	  $res=$id;
	}
	return($res);	
  }
  ...
  function add()
  {
  	...
  	(tipusu_nombre,tipusu_descripcion)
  	values
  	('$this->tipusu_nombre','$this->tipusu_descripcion')
va;
  	  $rs=&$this->con->Execute($sql);
  	  if($rs)
  	  {
  ...
  function update($id)
  {
    ...
    {
      //verificar nombre
      if($this->tipusu_nombre==$oAux->tipusu_nombre)
      {
        $datoNombre=$oAux->tipusu_nombre;
      }
      else 
      {
        $existe=$this->exist();
        if($existe=="0")
          $datoNombre=$this->tipusu_nombre;
        else 
          $datoNombre=$oAux->tipusu_nombre;  
      }
      $sql=<<<sql
        update tipousuario set
        tipusu_nombre='$datoNombre',
        tipusu_descripcion='$this->tipusu_descripcion' 
        where 
       	tipusu_id=$oAux->tipusu_id
sql;
      $rs=&$this->con->Execute($sql);
...
sql;
  ...
  {
  	if(strlen($orderby)==0)
  	  $cadOrderby=" order by tipusu_nombre ";
  	else 
  	  $cadOrderby=$orderby;
  	
  	$sql=<<<cad
		select tipusu_id,tipusu_nombre
		from tipousuario
		$cadOrderby
cad;
	return($sql);
  ...
  function htmlSelect($nombreCampo,$valorCampo,$estilo)
  {
  	...
  	$rs=&$this->con->Execute($this->sqlSelect("order by tipusu_nombre"));
  	$cadOpcion="";
  	while (!$rs->EOF) 
  	{
  	  $valor=$rs->fields[0];
...
cad;
  ...
  function cargar_dato($dato,$iou="i")			
  {
	  ...
	  {
        $this->tipusu_nombre=$dato[0];
	    $this->tipusu_descripcion=$dato[1];
...
va;
  	...
  	$sql=<<<va
  	  select tipusu_id,tipusu_nombre,tipusu_descripcion,tipusu_id 
	  from tipousuario 
	  order by tipusu_nombre
va;
	$rs=&$this->con->Execute($sql);
    if ($rs->EOF) 
	...
	{
	  $mainheaders=array("Elim.","Tipo de Usuario","Descripci&oacute;n","Modificar");
...
va;
  ...
  {
    $campo=array(
				array("etiqueta"=>"* Tipo de Usuario","nombre"=>"tNombre","tipo_campo"=>"text","sql"=>"","valor"=>""),
...
function adminUpd($formaAction,$principal,$id_aplicacion,$id_subaplicacion,$titulo,$id)  
  {
    $oAux=new c_tipousuario($this->con);
  	$oAux->info($id);
	$campo=array(
				array("etiqueta"=>"* tipousuario","nombre"=>"tNombre","tipo_campo"=>"text","sql"=>"","valor"=>$oAux->tipusu_nombre),
				array("etiqueta"=>"* Descripci&oacute;n","nombre"=>"tDescripcion","tipo_campo"=>"text","sql"=>"","valor"=>$oAux->tipusu_descripcion)
				);
...
va;
  ...
  function validaJs()
  {
	...
	function valida()
	{
  	  define('tNombre', 'string', 'tipousuario',1,100,document);
  	}
