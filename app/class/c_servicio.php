<?php
/**
 * Administrar la tabla servicio
 *
 */
include_once("class/c_interfaz.php");

class c_servicio implements c_interfaz 
{
  //atributos base
  var $ser_id;
  var $ser_nombre;
  ...
  { 
    $sql=<<<vic
	    select ser_id from servicio
	    where ser_nombre='$this->ser_nombre'
vic;
  ...
  { 
    ...
    $sql=<<<vic
	    select ser_id,ser_nombre,ser_descripcion,ser_id_padre 
	    from servicio
	    where ser_id=$id
vic;
	...
	else 
	{
	  ...
	  $this->ser_nombre=$rs->fields[1];
	  $this->ser_descripcion=$rs->fields[2];
	  $this->ser_id_padre=$rs->fields[3];
	  $res=$id;
	}
	return($res);	
  }
  ...
  function add()
  {
  	...
  	(ser_nombre,ser_descripcion,ser_id_padre)
  	values
  	('$this->ser_nombre','$this->ser_descripcion','$this->ser_id_padre')
va;
  	  $rs=&$this->con->Execute($sql);
  	  if($rs)
  	  {
  ...
  function update($id)
  {
    ...
    {
      //verificar nombre
      if($this->ser_nombre==$oAux->ser_nombre)
      {
        $datoNombre=$oAux->ser_nombre;
      }
      else 
      {
        $existe=$this->exist();
        if($existe=="0")
          $datoNombre=$this->ser_nombre;
        else 
          $datoNombre=$oAux->ser_nombre;  
      }
      $sql=<<<sql
        update servicio set
        ser_nombre='$datoNombre',
...
sql;
  ...
  {
  	if(strlen($orderby)==0)
  	  $cadOrderby=" order by ser_nombre ";
  	else 
  	  $cadOrderby=$orderby;
...
cad;
  ...
  function htmlSelect($nombreCampo,$valorCampo,$estilo)
  {
  	...
  	$rs=&$this->con->Execute($this->sqlSelect("order by ser_nombre"));
  	$cadOpcion="";
  	while (!$rs->EOF) 
  	{
  	  $valor=$rs->fields[0];
...
cad;
  ...
  function cargar_dato($dato,$iou="i")			
  {
	  ...
	  {
        $this->ser_nombre=$dato[0];
        $this->ser_descripcion=$dato[1];
...
va;
  	...
  	$sql=<<<va
  	  select ser_id,ser_nombre,ser_descripcion,ser_id 
	  from servicio 
	  order by ser_nombre
va;
	$rs=&$this->con->Execute($sql);
	...
	{
	  $mainheaders=array("Elim.","Servicio","Descripcion","Modificar");
...
va;
  ...
  {
    $campo=array(
				array("etiqueta"=>"* Servicio","nombre"=>"tNombre","tipo_campo"=>"text","sql"=>"","valor"=>""),
...
function adminUpd($formaAction,$principal,$id_aplicacion,$id_subaplicacion,$titulo,$id)  
  {
    $oAux=new c_servicio($this->con);
  	$oAux->info($id);
	$campo=array(
				array("etiqueta"=>"* Servicio","nombre"=>"tNombre","tipo_campo"=>"text","sql"=>"","valor"=>$oAux->ser_nombre),
				array("etiqueta"=>"* Descripcion","nombre"=>"tNivel","tipo_campo"=>"text","sql"=>"","valor"=>$oAux->ser_descripcion),
				);
...
va;
  ...
  function validaJs()
  {
	...
	function valida()
	{
  	  define('tNombre', 'string', 'servicio',1,100,document);
  	}
